...
export default function DeliveryMethodsList({ deliveryMethods: providedDeliveryMethods, weight: providedWeight, weightUnit, sort }: DeliveryMethodsListProps) {
        ...
        if (transportationMethod && transportationMethod.includes("Air"))
            return (
                <div className={"icon-box"}>
                    <img src={"https://trtshopping.com/wp-content/uploads/2023/06/air-mode.svg"} alt={""} />
                </div>
            );
        else if (transportationMethod && transportationMethod.includes("Ocean"))
            return (
                <div className={"icon-box"}>
                    <img src={"https://trtshopping.com/wp-content/uploads/2023/06/ocean-mode.svg"} alt={""} />
                </div>
            );
                ...
                if (rates?.length > 1) {
                    ...
                    rates?.forEach((rate: any) => {
                        if (rate != null && rate[1] != null) {
                            if (rate[1] < selectedMinRate) selectedMinRate = rate[1];
                    ...
                    if (rates[0] != null && rates[0][1] != null) return convertToUnit(rates[0][1], deliveryMethod?.tariff?.unitType?.toLowerCase(), weightUnit)?.toFixed(2);
                }
            }
        } else return convertToUnit(deliveryMethod?.tariff?.ratePer, deliveryMethod?.tariff?.unitType?.toLowerCase(), weightUnit)?.toFixed(2);
    };
    ...
    const getMethodTotalAmount = (deliveryMethod: any, weight: string | null) => {
        ...
        const result = rateNumber * weightNumber;
        if (!isNaN(result) && weightNumber > 0.001) {
            if (deliveryMethod.tariff.minimum && result < deliveryMethod.tariff.minimum) {
                return deliveryMethod.tariff.minimum.toFixed(2);
            } else if (deliveryMethod.tariff.maximum && result > deliveryMethod.tariff.maximum) {
                return deliveryMethod.tariff.maximum.toFixed(2);
            }
    ...
    return (
        <div className={"delivery-methods"}>
            {deliveryMethods?.map((deliveryMethod, index) => {
                const isNotPickupLocationRate = !deliveryMethod.customValues["is_pickup_location_rate"];
                return (
                    <>
                        <div className={"delivery-method"}>
                            <div className={"delivery-method-transportation-carrier"}>
                                <div className={"icons"}>
                            ...
                            <div className={"delivery-method-method-rate"}>
                                <div>
                                    {deliveryMethod?.customValues?.["service_description_" + i18n.language ?? "en"]}
                                    {" - "}
                                    {t("deliveryMethods.from")} {deliveryMethod?.transitDaysMin} {t("deliveryMethods.upto")} {deliveryMethod?.transitDaysMax} {t("deliveryMethods.days")}
                                </div>
                                {getMethodTotalAmount(deliveryMethod, providedWeight) ? (
                                    <div className={"delivery-method-rate"}>${getMethodTotalAmount(deliveryMethod, providedWeight)}</div>
                                ) : (
                                    <div className={"delivery-method-rate"}>
                        ...
                        {deliveryMethods?.length - 1 > index ? <Divider flexItem /> : null}
                    </>
                );
            })}
        </div>
